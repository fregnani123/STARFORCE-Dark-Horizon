 ...
 * Agora salva o progresso e estrelas capturadas durante a missão.
 */
export function derrotouBoss() {
    ...
    if (playerShip && playerShip.isAlive) {
        ...
        try {
            ...
            console.log(`Salvando missão ${currentMission} com ${starsEarned} estrelas...`);
            
            // Atualiza para próxima missão
            updateMissionProgress(currentMission + 1).catch(err => 
                console.error("Erro ao atualizar missão:", err)
            );
            
            // Salva as estrelas capturadas
            if (starsEarned > 0) {
                addStars(starsEarned, currentMission).catch(err => 
                    console.error("Erro ao salvar estrelas:", err)
                );
